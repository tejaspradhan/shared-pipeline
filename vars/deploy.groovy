...
def call(url)
{
  ...
  remote.each{
        item -> L:{
          ...
          sshCommand remote : item, command: 'cd /home/'+"${item.name}"+'/'+"${appname}"+'/target; TEMP=`find *.jar | awk \'{print $1}\'`; PID=`ps -eaf | grep "$TEMP" | grep -v grep | awk \'{print $2}\'`; if [[ "" !=  "$PID" ]]; then echo "found $PID and killing it!!" ; kill -9 $PID; fi'
